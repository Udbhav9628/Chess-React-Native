...
const ChessComp = () => {
	...
	useEffect(() => {
		if (
			contextApi?.mySocketId?.length <= 0 ||
			contextApi?.mySocketId?.length === undefined
		) {
			setShowModal(true);
		}
	}, []);
	...
	const handlePress = () => {
		if (inputFocused) {
			if (contextApi?.whomToCall?.length != 0) {
				...
				ToastAndroid.show(
					'Please Enter Connection id first !',
					ToastAndroid.SHORT
				);
			}
		} else {
			...
			ToastAndroid.show(
				'Copied!   Share this id With Whom You want to Connect',
				ToastAndroid.SHORT
			);
		}
	};
	...
	return (
		<GestureHandlerRootView>
			<View>
				{contextApi?.opponentSocketId?.length > 0 ? (
					<View style={styles.container}>
						...
						>
							...
							>
								{contextApi?.yourSide === 'b' ? 'You' : 'Opponent'}
							</Text>
							<Image
								style={{
									width: 50,
						...
						>
							...
							>
								{contextApi?.yourSide === 'w' ? 'You' : 'Opponent'}
							</Text>
						</View>
					</View>
				) : (
					<View style={styles.containerTextInput}>
						...
						>
							<Text style={styles.inputText}>
								...
								{inputFocused
									? 'Enter Connection id'
									: 'Touch to Enter Player Connection id '}
							</Text>
							<TextInput
								onFocus={handleFocus}
								...
								value={
									inputFocused
										? contextApi?.whomToCall?.length
										: contextApi?.mySocketId
								}
						...
						<TouchableOpacity
							onPress={handlePress}
							disabled={contextApi?.Loading}
							style={{
								...styles.button,
								backgroundColor:
									contextApi?.Loading || contextApi?.whomToCall?.length === 0
										? '#944048'
										: '#5e030c',
							}}
						>
							{contextApi?.Loading ? (
								...
								<Text style={styles.buttonText}>
									{inputFocused ? 'Connect' : 'Copy Your id'}
								</Text>
							)}
						</TouchableOpacity>
					</View>
